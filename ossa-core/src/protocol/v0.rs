...
fn initial_miniprotocols<StoreId, Hash, HeaderId, Header>(
    ...
    vec![
        ...
        MiniProtocols::Manager(Manager::new(
            Party::Client,
            args.peer_id,
            args.active_stores.clone(),
            client_chan,
            1,
            multiplexer_cmd_send.clone(),
        )),
        MiniProtocols::Manager(Manager::new(
            Party::Server,
            args.peer_id,
...
impl<StoreId> Into<MsgStoreMetadataHeader<StoreId>> for StoreMetadataHeaderRequest<StoreId> {
    fn into(self) -> MsgStoreMetadataHeader<StoreId> {
        MsgStoreMetadataHeader::Request(self)
    }
}
impl<StoreId> Into<MsgStoreMetadataHeader<StoreId>> for StoreMetadataHeaderResponse<StoreId> {
    fn into(self) -> MsgStoreMetadataHeader<StoreId> {
        MsgStoreMetadataHeader::Response(self)
    }
}
impl<StoreId> TryInto<StoreMetadataHeaderRequest<StoreId>> for MsgStoreMetadataHeader<StoreId> {
    type Error = ();
...
impl<StoreId> TryInto<StoreMetadataHeaderResponse<StoreId>> for MsgStoreMetadataHeader<StoreId> {
    type Error = ();
